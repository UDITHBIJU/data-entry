...
export class RecordsService {
  ...
  async create(createRecordDto: CreateRecordDto,userId:string){
    ...
    async findAll(userId:string,page:number,limit:number,search?:string){
        const user = await this.usersService.findOne(userId);
        const query = this.recordsRepository.createQueryBuilder('record').where('record.assignedToId = :userId', { userId: user.id })
        .andWhere('record.isLocked = :isLocked', { isLocked: false }).skip((page - 1) * limit).take(limit);

        if (search) {
            query.andWhere('record.searchVector @@ to_tsquery(:search)', { search:search + ':*' });
        }
    ...
    async lockRecord(id: string, userId: string) {
        ...
        setTimeout(() => 
            this.unlockRecord(id),10*60*1000);
    ...
    async unlockRecord(id: string) {
        ...
        if (record && record.isLocked) {
            ...
            if (auditUserId) {
              ...
              console.warn(
                `No assignedTo or lockedBy for unlocked record ID ${id}`,
              );
            }
        }
    }
    async update(id: string, updateRecordDto: UpdateRecordDto, userId: string) {
        ...
        await this.createAuditLog(
            'update', this.getChanges(oldData, updatedRecord),
            updatedRecord,
            userId,
        );
